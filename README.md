...
PM FIX 화면설계서와 **「팀원B_양정현_v4 — 시스템·AI·RAG 구조 설계」**의 상태·데이터 계약을 기준으로 구현한 역할 연계형 HTML 프로토타입입니다.
...
| 모델 검증 범위 | `mvp_primary`·`expansion_secondary`·`unsupported`·`archived` |
| 역할 화면 | 고객·상담원·방문기사·운영직원 |

`WPUJAC104DWH / WPU-JAC104D`만 기본 MVP AI·RAG 시뮬레이션을 허용합니다. `WPUIAC425SNW`는 제품 등록·지원범위 확인 화면에서는 **후속 확장**으로 표시하지만 기본 검색에서 차단하며, `WPUIAC506`은 **보관 모델**, 그 밖의 모델은 **미지원**으로 분류해 제품 상담으로 연결합니다. IoT 케어, 앱 연결, 원격 관리, 얼음 기능과 AI 스마트 준비는 이 문서의 MVP 검색 계약에 포함하지 않았습니다.
...
- 미지원·후속 확장·보관 제품: AI·RAG 실행 없이 지속적인 제품 상담 대체 화면, 중복 요청 방지, 상담 결과 확인
...
- AI 원본 요약과 상담사 수정본을 별도 필드·별도 카드로 표시하고 수정 이력 저장
- `scope_role`, `applicability`, `allowed_use`, `verification_status`를 포함한 근거 메타데이터 조회
...
- AI 원본·상담사 수정본·상담 인계·현재 사용 제한을 구분한 방문 전 리포트
- 실제 원인, 조치, 부품, 사용 안내, 제한 기능, 판단 근거, 다음 행동, 재방문·후속상담·특이사항
- 고객 서명 확인, 방문 완료 후 케어 이력·최근 관리일·다음 일정 협의 상태 반영, 방문 경로 담당 기사 최종 완료
...
활성 화면은 `assets/js/data/seed-data.js`의 `EvidenceCardDTO`만 사용합니다.
...
- 현재 매뉴얼 데이터에 원본 정책값이 없는 `allowedUse`는 임의 생성하지 않고 `null`로 유지
...
- `requirements-v11.test.mjs`: 모델 범위·6개 시나리오·EvidenceCardDTO·새 모듈 로드 순서·화면·이벤트·역할 분리·반응형 계약
- `state-flow.test.mjs`: 독립 사전 문진, 제품 검증 차단, 일반 안내, 위험별 사용 제한, `NO_EVIDENCE`, AI 원본·사람 수정본, 상담·방문 연계, 케어 이력, 알림, 최종 완료, 재방문, 일정 변경, 중복 이벤트, 상태 충돌
...
기존 `mock-data.js`, `store.js`, `customer.js`, `stakeholder.js`, `workflow-config.js` 등은 이전 프로토타입 이력 보존용이며 **현재 5개 활성 HTML에서는 로드하지 않습니다.** 신규 수정은 위 활성 구조만 대상으로 합니다.
